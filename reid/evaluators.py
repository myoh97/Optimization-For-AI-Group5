...
def pairwise_distance_comp(features_q, features_g, query=None, gallery=None, metric=None):
    x = torch.cat([features_q[f].unsqueeze(0) for f, _, _,_ in query], 0)
    y = torch.cat([features_g[f].unsqueeze(0) for f, _, _,_ in gallery], 0)
...
def evaluate_all(query_features, gallery_features, distmat, query=None, gallery=None,
                 ...
                 cmc_topk=(1, 5, 10), cmc_flag=False, cuhk03=False, save=None):
    ...
    if cuhk03:
        ...
        mAP = mean_ap(distmat, query_ids, gallery_ids, query_cams, gallery_cams, query, gallery, save)
...
@torch.no_grad()
def extract_gallery(model, gallery_loader):
    model.eval()
    batch_time = AverageMeter()
    data_time = AverageMeter()
    ...
    with torch.no_grad():
        for i, (imgs, fnames, pids, cids, domians) in enumerate(gallery_loader):
            data_time.update(time.time() - end)
...
class Evaluator(object):
    ...
    def evaluate_compatible(self, query_loader, gallery_features, query, gallery, cmc_flag=True, save=None):
        query_features, _ = extract_features(self.model, query_loader)
        
        distmat, query_features, gallery_features = pairwise_distance_comp(query_features, gallery_features, query, gallery, metric=None)
        results = evaluate_all(query_features, gallery_features, distmat, query=query, gallery=gallery, cmc_flag=cmc_flag, save=save)
