...
class ResNet(nn.Module):
    def __init__(self, last_stride, bn_norm, with_ibn, with_se, block, num_class, layers, args=None):
        self.inplanes = 64
        super().__init__()
    ...
    # input shape : (256, 3, 256, 128)
    def forward(self, x, domains=None, training_phase=None, fkd=False,):

        x = self.conv1(x)   # shape: (256, 64, 128, 64)
        x = self.bn1(x)     # shape: (256, 64, 128, 64)
        x = self.relu(x)    # shape: (256, 64, 128, 64)
        x = self.maxpool(x) # shape: (256, 64, 64, 32)

        # layer 1
        x = self.layer1(x) # shape: (256, 64, 64, 32)
        x = self.layer2(x) # shape: (256, 128, 32, 16)
        x = self.layer3(x) # shape: (256, 256, 16, 8)
        x = self.layer4(x) # shape: (256, 512, 16, 8)

        global_feat = self.pooling_layer(x)

        bn_feat = self.bottleneck(global_feat) # bn
...
def build_resnet_backbone(num_class, depth, pretrain=True, args=None):
    ...
    model = ResNet(1, 'BN', False, False, block, num_class, num_blocks_per_stage, args=args)
    if pretrain:
        cached_file = './pretrained/resnet50-19c8e357.pth'
